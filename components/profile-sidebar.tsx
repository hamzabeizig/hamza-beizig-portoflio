...
export function ProfileSidebar({ data = profileData }: ProfileSidebarProps) {
  return (
    <aside className="w-full lg:w-80 bg-white/80 dark:bg-black/80 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-white/10 p-4 md:p-6 lg:sticky lg:top-8 lg:h-fit lg:min-h-[95vh] shadow-2xl transition-all duration-500 hover:bg-white/90 dark:hover:bg-black/90">
      {/* Profile Image */}
      <div className="flex flex-col items-center">
        <div className="relative w-[25vh] aspect-square mb-6 md:mb-8">          <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-accent/20 via-accent/5 to-transparent animate-pulse-slow" />
          <div className="absolute inset-[2px] rounded-3xl bg-secondary overflow-hidden w-full h-full">
            <img
        ...
        <p className="text-xs md:text-sm text-muted-foreground bg-secondary mt-3 px-3 md:px-4 py-1 rounded-lg text-center">
          {data.title}
        </p>
      </div>
