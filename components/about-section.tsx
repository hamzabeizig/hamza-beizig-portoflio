import { PenTool, Code, BarChart3, CheckCircle, Zap, Smartphone } from 'lucide-react'
...
export function AboutSection({ data = aboutData }: AboutSectionProps) {
  return (
    <div className="space-y-8 md:space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          {data.services.map((service, index) => {
            ...
            return (
              <div
                key={index}
                className="flex gap-3 md:gap-4 p-4 md:p-6 bg-secondary/40 backdrop-blur-md rounded-xl md:rounded-2xl border border-white/10 dark:border-white/5 hover:border-accent hover:shadow-lg hover:shadow-accent/10 hover:-translate-y-1 transition-all duration-300"
              >
        ...
        <div className="relative overflow-hidden">
          <div className="flex gap-3 md:gap-4 animate-marquee">
            {[...data.testimonials, ...data.testimonials].map((testimonial, index) => (
              <div key={index} className="flex-shrink-0 w-72 md:w-80 p-4 md:p-6 bg-secondary/40 backdrop-blur-md rounded-xl md:rounded-2xl border border-white/10 dark:border-white/5 hover:border-accent/50 hover:shadow-lg transition-all duration-300 group">
      ...
      <div>
        <h3 className="text-xl md:text-2xl font-bold text-foreground mb-6">Companies I Worked With</h3>
        <div className="relative overflow-hidden py-4">
          <div className="flex gap-4 md:gap-6 animate-marquee-slow">
            {[...data.clients, ...data.clients].map((client, index) => (
              <div
                key={index}
                className="flex-shrink-0 w-32 h-20 md:w-40 md:h-24 bg-secondary/40 backdrop-blur-md rounded-xl md:rounded-2xl border border-white/10 dark:border-white/5 flex items-center justify-center p-4 md:p-6 hover:border-accent hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
              >
                <img
